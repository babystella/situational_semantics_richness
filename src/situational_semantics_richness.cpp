...
void processcallback(const radiation::sievertConstPtr msg1, const risk_assessment::riskConstPtr msg2, const noise_detection::noiseConstPtr msg3)
{ 
    // ros::Time msg_time = msg1->header.stamp;
...
int main(int argc, char *argv[])
{
	...
	message_filters::Subscriber<noise_detection::noise> laser_sub(nh, "/noise_evaluation", 1);
	typedef message_filters::sync_policies::ApproximateTime<radiation::sievert, risk_assessment::risk, noise_detection::noise> MySyncPolicy;
	message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), sv_sub, risk_sub, laser_sub);
    sync.registerCallback(boost::bind(&processcallback, _1, _2, _3));
